...
3.  Now we can try several command in the Terminal: `git status`, `git commit`, `git push`, `git fetch`, and `git push`. See [here](https://rviews.rstudio.com/2020/04/23/10-commands-to-get-started-with-git/) if you want to know more than this. Now, let's try to type something below in your collaborators repository, then commit, and push it. Check your repo as well.
...
# basic example
ggplot(cpsemployed, aes(x = age, y = race, fill = race)) +
  geom_density_ridges() +
  theme_ridges() + 
...
# Area + contour
ggplot(cpsemployed, aes(x=educ, y=earnwk) ) +
